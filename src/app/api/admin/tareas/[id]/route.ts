...
// PATCH  /api/admin/tareas/[id] — actualizar tarea (estado, campos)
...
export async function PATCH(
  ...
  try {
    ...
    if (body.estado) { updates.push('estado = ?'); args.push(body.estado); }
    ...
    // Si se completa, setear fechaCompletada
    ...
    if (body.estado === 'COMPLETADA') {
      const tareaRes = await libsql.execute({
        sql: 'SELECT titulo, contactoId FROM Tarea WHERE id = ?',
        args: [id],
      });
      const contactoId = (tareaRes.rows[0] as Record<string, unknown>)?.contactoId as string | null;
      if (contactoId) {
        ...
        await libsql.execute({
          ...
          args: [
            actId, contactoId, session.user.id,
            `Tarea completada: ${(tareaRes.rows[0] as Record<string, unknown>)?.titulo ?? ''}`,
          ],
        });
      }
    }
