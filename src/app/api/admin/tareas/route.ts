// GET  /api/admin/tareas — lista paginada de tareas con filtros
...
export async function GET(request: Request) {
  try {
    ...
    // Tareas con JOIN (Contacto tiene nombre/telefono)
    const result = await libsql.execute({
      sql: `SELECT t.id, t.titulo, t.descripcion, t.tipo, t.estado, t.prioridad,
          t.fechaLimite, t.fechaCompletada, t.asignadoA, t.asignadoPor, t.contactoId,
          t.empresaId, t.createdAt, t.updatedAt,
...
export async function POST(request: Request) {
  try {
    ...
    if (!titulo || !asignadoA) {
      return new Response('Título y vendedor son requeridos', { status: 400 });
    }
    ...
    if (!empresaId) {
      return new Response('El vendedor no tiene empresa asignada', { status: 400 });
    }

    const tareaId = 'tarea_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
    // fechaLimite es NOT NULL — default +24h si no se provee
