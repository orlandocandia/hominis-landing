// PUT /api/profile/phones/[id] — update a phone
// DELETE /api/profile/phones/[id] — delete a phone
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Verify ownership
    const existing = await libsql.execute({ sql: 'SELECT userId FROM UserPhone WHERE id = ?', args: [id] });
    ...
    if (isPrimary) {
      ...
      await libsql.execute({ sql: 'UPDATE UserPhone SET isPrimary = 0 WHERE userId = ?', args: [ownerId] });
    }
    await libsql.execute({
      sql: `UPDATE UserPhone SET phoneNumber = ?, phoneType = ?, isPrimary = ?, isWhatsapp = ?, notes = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?`,
      args: [phoneNumber, phoneType, isPrimary ? 1 : 0, isWhatsapp ? 1 : 0, notes || null, id],
    });
    return NextResponse.json({ ok: true });
  } catch (e: any) {
    if (e.message?.includes('UNIQUE')) return NextResponse.json({ error: 'Ese teléfono ya está registrado' }, { status: 409 });
    console.error('[phones PUT] error:', e);
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await libsql.execute({ sql: 'DELETE FROM UserPhone WHERE id = ?', args: [id] });
    return NextResponse.json({ ok: true });
  } catch (e: any) {
    console.error('[phones DELETE] error:', e);
    return NextResponse.json({ error: 'Error del servidor' }, { status: 500 });
  }
}
