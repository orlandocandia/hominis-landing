...
// /admin/empresas — Panel de administración de empresas (multiempresa Fase 4)
import { useState, useEffect, useCallback } from 'react';
import { Building2, Plus, Trash2, Edit3, X, Loader2, Phone, Mail, MapPin } from 'lucide-react';
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <Label>Rubro</Label>
                <Select value={form.rubro} onValueChange={(v) => setForm({ ...form, rubro: v })}>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {emp.isActive ? (
                        <Badge variant="secondary" className="border-transparent bg-emerald-500/15 text-emerald-600">
                          Activa
                        </Badge>
                      ) : (
                    ...
                    <Badge variant="outline" className="mt-1 text-[11px]">{emp.rubro}</Badge>
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
                ...
                <div className="mt-3 space-y-1 text-sm text-muted-foreground">
                  ...
                  {emp.telefono && (
                    <p className="flex items-center gap-2">
                      <Phone className="h-3.5 w-3.5 shrink-0" /> {emp.telefono}
                    </p>
                  )}
                  {emp.direccion && (
                    <p className="flex items-center gap-2 truncate">
                      <MapPin className="h-3.5 w-3.5 shrink-0" /> {emp.direccion}
                    </p>
                  )}
                  {emp.cuit && (
                    <p className="text-xs text-muted-foreground/70">CUIT: {emp.cuit}</p>
                  )}
                </div>
              </CardContent>
            </Card>
