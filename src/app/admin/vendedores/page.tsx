...
import {
  ...
  Users, Building2, Mail, Phone, Eye, Trash2, CheckCircle2, XCircle,
} from 'lucide-react';
...
export default function VendedoresPage() {
  ...
  const handleDelete = async (id: string, nombre: string) => {
    if (!confirm(`¿Eliminar al vendedor "${nombre}"? Esta acción no se puede deshacer.`)) return;
    try {
  ...
  const handleToggleActive = async (id: string, activo: boolean) => {
    try {
      ...
      if (!res.ok) throw new Error();
      toast.success(activo ? 'Vendedor desactivado' : 'Vendedor activado');
      fetchVendedores();
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button asChild className="gap-2">
          <Link href="/admin/vendedores/nuevo"><Plus className="h-4 w-4" /> Nuevo Vendedor</Link>
        </Button>
      </div>
